...
2026-07 現在、M0〜M4 の Issue #1〜#25、Phase 2 の #26、タイムスライダーの #27、任意地点出発の #28 は実装済み。以下は実装時の設計判断と
...
| 初期ロード | データ合計 gzip 後 **≤1.5MB**(CI サイズゲート)。承認済みフィード実測 約 0.50MB |
...
- 承認済みブラウザデータ実測: gzip 後 timetable 約 0.36MB、stops 約 0.14MB、manifest を含む合計 約 0.50MB
...
├── apps/
│   └── web/                 # Vite + vanilla TypeScript + MapLibre GL JS
├── docs/
│   ├── PLAN.md              # 本ドキュメント
...
`manifest + stops + timetable` の gzip 合計 **≤1.5MB** を pipeline の出力検証で強制。超過したらビルド失敗。承認済みフィードの実測は約 0.50MB で余裕をもたせている。
...
各レイヤは「そのサービス日に有効な service_id 集合」でフィルタした上で、
`globalMinute = tripMinute - minuteOffset` として単一のタイムライン上で探索する。
...
```ts
// packages/raptor/src/core.ts

export type Minutes = number; // サービス日 0:00 からの分

export interface EarliestArrivalQuery {
  readonly kind: 'earliestArrival';
  readonly origins: readonly { readonly stopIndex: number; readonly departure: Minutes }[];
  readonly serviceDate: string; // 'YYYYMMDD'
...
### 任意地点からの初期徒歩接続(apps/web、Issue #28)

- 地図クリックまたはブラウザの現在地を出発地点として選択できる
- 停留所データ読込時に約800mセルの空間グリッドを一度構築し、地点の周囲9セルだけを検索する
- 地点から直線距離 **800m 以内**の全停留所を複数 origin とし、初期到着時刻に
  `距離 / 80m/分 + 1分`を加える
- 30分/60分ポリゴンには地点の歩行バッファも含め、地点自体が到達圏から欠落しないようにする
- 緯度経度は共有URLの `originLat` / `originLon` に保存し、再読込時に同じ探索を復元する
...
Issue #26 は実装済みで、契約と検証結果を [`PHASE2.md`](./PHASE2.md) に集約。Issue #27 は
5 分刻み、180ms debounce、Worker cancellation による逐次計算として実装し、事前計算キャッシュは
採用していない。Issue #28 は800m空間グリッドによる複数 origin として実装済み。#29〜#30 は
